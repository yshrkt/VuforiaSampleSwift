...
//  Modified by Byron Coetsee on 2016/10/01
...
- (instancetype)init {
	...
	[NSException raise:NSGenericException format:@"Disabled. Use +[[%@ alloc] %@] instead", NSStringFromClass([self class]), NSStringFromSelector(@selector(initWithLicenseKey:dataSetFile:))];
	return nil;
}
...
- (void)initTracker {
	...
	Vuforia::ObjectTracker* objectTrackerBase = static_cast<Vuforia::ObjectTracker*>(trackerBase);
	if (trackerBase == NULL)
	{
		[self.delegate vuforiaManager: self didFailToPreparingWithError:[self buildErrorWithCode:VuforiaError_InitTrackers]];
		//		return;
	}
	objectTrackerBase->start();
	
	Vuforia::TargetFinder* targetFinder = objectTrackerBase->getTargetFinder();
...
- (void)loadTrackerData {
	...
	if (targetFinder->startInit("6577f6ef36be561bde32216a4f7546ac3d681e1b", "be5a1de0ca0a4dfe898cf50d171f7d0742b4cecd"))
	{
...
- (void) Vuforia_onUpdate:(Vuforia::State *) state {
	...
	const Vuforia::TargetSearchResult* passingResult = nil;
	...
	else if (statusCode == Vuforia::TargetFinder::UPDATE_RESULTS_AVAILABLE)
	{
		...
		for (int i = 0; i < finder->getResultCount(); ++i)
		{
			...
			if (result->getTrackingRating() > 0)
			{
				...
				Vuforia::ImageTarget* imageTarget = finder->enableTracking(*result);
				Vuforia::Trackable* newTrackable = imageTarget;
				
				if (newTrackable != 0)
				{
					char* userData = const_cast<char*>(result->getMetaData());
					newTrackable->setUserData(userData);
